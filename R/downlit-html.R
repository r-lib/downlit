...
downlit_html_node <- function(x, classes = classes_pandoc()) {
  ...
  # .r-chunk used pkgdown Rmds - need to preserve classes
  xpath_pkgdown_rmd <- ".//div[contains(@class, 'r-chunk')]//pre"
  tweak_children(x, xpath_pkgdown_rmd, highlight,
    pre_class = "TEMP",
    classes = classes,
    code = TRUE,
    preserve_class = TRUE
...
tweak_children <- function(node, xpath, fun, ...,
  ...
  if (preserve_class) {
    for (i in seq_along(old)) {
      xml2::xml_attr(new[[i]], "class") <- xml2::xml_attr(old[[i]], "class")
    }
  }
